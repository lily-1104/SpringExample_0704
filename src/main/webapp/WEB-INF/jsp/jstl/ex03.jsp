...
<body>
	...
	<h4><fmt:formatNumber value="${number }" type="number" groupingUsed="false" /></h4>
						<%-- groupingUsed는 true가 기본값, false로 하면 value 값에서 ,가 사라짐 --%>
	
	
	
	<h2>통화 표현</h2>
	<h4>원 : <fmt:formatNumber value="${number }" type="currency" /></h4>
			<%-- "currency" 때문에 number 앞에 원(통화) 표시 붙음--%>
	...
	<h4> 3.14 </h4>
	<h4> <fmt:formatNumber value="3.14" pattern="0.0000" /> </h4>	<%-- 3.1400 출력 --%>
	<h4> <fmt:formatNumber value="3.14" pattern="#.####" /> </h4>	<%-- 3.14 출력 --%>
	<h4> <fmt:formatNumber value="3.141952" pattern="#.####" /> </h4>	<%-- 3.142 출력 --%>  
	
	
	
	<h2> 날짜 표현하기 </h2>
	
		<%-- Controller ex03 작성 후 가능 --%>
	...
	<h2> 문자열 Date 객체로 변환</h2>
		...
		<%-- 2022년 07월 18일 20시 15분 35초 --%>
	<%-- 날짜 정보가 문자열 형태로 전달되었을 때, 포맷을 바꾸고 싶을 때 --%>
